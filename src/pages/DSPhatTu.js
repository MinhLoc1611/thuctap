...
import { getArrPhatTuAction, setPhatTuEditAction } from "../redux/actions/userAction";
...
export default function DSPhatTu() {
    ...
    const {arrPhatTu,param,phatTuEdit} = useSelector(state => state.userReducer)
    ...
    const setPage = () => {
        ...
        for (let i = 0; i < totalPage; i++) {
            options.push({
                value: i + 1,
                label: `Hiển thị ${i * pageSize + 1} - ${(i + 1) * pageSize > totalCount ? totalCount : (i + 1) * pageSize
                    } of ${totalCount}`,
            });
        }
        return options;
